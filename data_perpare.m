...
%% ==================== 1. 配置参数 ==========================
config = struct();
config.simulation_hours = 2;    % 仿真时长 2 小时
...
for i = 1:length(faulty_machines)
    ...
    fault_start_ratio = 0.4 + 0.3*rand();
...
for mid = 1:config.n_machines
    ...
    temperature(1) = thermal_model.T_ambient(mid);
    ideal_temperature(1) = thermal_model.T_ambient(mid);
    ...
    for t = 2:N_steps
        ...
        ideal_temperature(t) = prev_temp_ideal + dTdt_ideal * config.dt;
        ...
        if is_faulty && fault_types(fault_idx) == 3
        ...
        if is_faulty && fault_types(fault_idx) == 2
        ...
        nozzle_position_z(t) = nozzle_position_z(t-1) + (target_z > nozzle_position_z(t-1)) * (target_z - nozzle_position_z(t-1));
        
        % 挤出压力
        pressure_multiplier = 1.0;
        if is_faulty && fault_types(fault_idx) == 1
            pressure_multiplier = 1.8 + 0.4*rand();
        end
        movement_speed = sqrt((target_x - nozzle_position_x(t-1))^2 + (target_y - nozzle_position_y(t-1))^2) / config.dt;
        ...
        extrusion_pressure(t) = base_pressure * pressure_multiplier * ...
            (0.6 + 0.25*speed_factor + 0.15*temp_factor) * print_quality.extrusion_multiplier(mid);
        
        % 矫正信号（理想 - 实际）
        correction_signal_x(t) = ideal_target_x - target_x;
        correction_signal_y(t) = ideal_target_y - target_y;
        correction_signal_temp(t) = thermal_model.T_target(mid) - temperature(t);
        
        % 打印质量
        vibration_magnitude = sqrt(new_disp_x^2 + new_disp_y^2);
        temp_stability = abs(temperature(t) - thermal_model.T_target(mid));
        base_quality = 1.0;
    ...
    machine_df = table(...
        ...
        motor_current_y, ...
        extrusion_pressure, ...
        print_quality_metric, ...
        ideal_position_x, ...
        ideal_position_y, ...
        correction_signal_x, ...
        correction_signal_y, ...
        correction_signal_temp, ...
        repmat(is_faulty, N_steps, 1), ...
...
% 提取数据
nozzle_position_x = zeros(height(all_data)/length(load_example), length(load_example));
nozzle_position_y = zeros(height(all_data)/length(load_example), length(load_example));
ideal_position_x = zeros(height(all_data)/length(load_example), length(load_example));
ideal_position_y = zeros(height(all_data)/length(load_example), length(load_example));
% 按机器拆分
...
subplot(2,3,2);
plot(time_vector(1:min(2000, length(time_vector))), correction_signal_x(1:min(2000, length(time_vector)))*1000, 'b-');
hold on;
plot(time_vector(1:min(2000, length(time_vector))), correction_signal_y(1:min(2000, length(time_vector)))*1000, 'r--');
title('矫正信号 (机器1)');
